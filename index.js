...
const TIKTOK_FOLLOWERS_LIFETIME_SERVICE_ID = 11074; // 👥 متابعين تيك توك (مدى الحياة)
const TIKTOK_LIKES_SERVICE_ID = 10927; // ❤️ اعجابات تيك توك (اضافة جديدة)
const TIKTOK_LIKES_CHEAP_SERVICE_ID = 11059; // ❤️ اعجابات تيك توك (رخيصة)
// 📸 انستقرام
const IG_LIKES_SERVICE_ID = 10947; // ❤️ لايكات انستقرام
...
// 🎮 ببجي
const PUBG_UC_60_SERVICE_ID = 11076; // 🎮 شدات ببجي 60 UC
const PUBG_UC_325_SERVICE_ID = 11077; // 🎮 شدات ببجي 325 UC
...
const ORDER_TYPE_TO_SERVICE_ID = {
  ...
  ttlikes_cheap: TIKTOK_LIKES_CHEAP_SERVICE_ID,
...
// 🎮 ببجي
pubg60: PUBG_UC_60_SERVICE_ID,
pubg325: PUBG_UC_325_SERVICE_ID,
...
function servicesKeyboard() {
  return {
    inline_keyboard: [
      ...
      [{ text: "❤️ اعجابات تيك توك (رخيصة)", callback_data: "NAV:SVC_TT_LIKES_CHEAP" }],
      ...
      // 🎮 ببجي
      [{ text: "🎮 شدات ببجي 60 UC", callback_data: "NAV:SVC_PUBG_60" }],
      [{ text: "🎮 شدات ببجي 325 UC", callback_data: "NAV:SVC_PUBG_325" }],
...
const ttFollowersPrices = { 500: 7000, 1000: 11000, 100: 3000 }; // مثال (غيرها حسب سعر نقاطك)
...
// 🎮 ببجي (شدات)
const pubg60Prices = { 60: 9000 };     // 💰 غير السعر حسبك
const pubg325Prices = { 325: 35000 };  // 💰 غير السعر حسبك
...
// 🎬 تيك توك (اعجابات رخيصة)
const ttLikesCheapPrices = { 10: 15, 20: 25, 30: 35, 40: 45, 50: 55, 1000: 1200, 10000: 6000 };
...
{ text: "📨 تواصل للشراء", url: "https://t.me/GWVEW" }
    ...
    // 🎮 ببجي
if (action === "SVC_PUBG_60") {
  return editOrSend(chatId, "🎮 شدات ببجي 60 UC\n\nالسعر: " + pubg60Prices[60] + " نقطة", {
    inline_keyboard: [
      [{ text: "شراء 60 UC", callback_data: "BUY:PUBG60:60" }],
      [{ text: "⬅️ رجوع", callback_data: "NAV:SERVICES" }],
    ],
  });
}

if (action === "SVC_PUBG_325") {
  return editOrSend(chatId, "🎮 شدات ببجي 325 UC\n\nالسعر: " + pubg325Prices[325] + " نقطة", {
    inline_keyboard: [
      [{ text: "شراء 325 UC", callback_data: "BUY:PUBG325:325" }],
      [{ text: "⬅️ رجوع", callback_data: "NAV:SERVICES" }],
    ],
  });
}
    ...
    if (action === "SVC_TT_LIKES_CHEAP") return showQtyMenu(chatId, "❤️ اعجابات تيك توك (رخيصة)\nاختر الكمية:", "BUY:TTLIKESCHEAP", ttLikesCheapPrices, "NAV:SERVICES");
  ...
  if (data.startsWith("BUY:")) {
    ...
    if (type === "TTLIKESCHEAP") { cost = ttLikesCheapPrices[qty] || 0; askText = "🔗 أرسل رابط:"; orderType = "ttlikes_cheap"; }
  ...
  if (type === "PUBG60") {
  cost = pubg60Prices[qty] || 0;
  askText = `⚡️ شحن شدات PUBG Mobile

🔴 اطلب بأيدي حسابك داخل اللعبة
🔴 كيفية العثور على مُعرّف اللاعب الخاص بك:
1. افتح PUBG Mobile
2. انقر على صورتك (أعلى اليمين)
3. ستجد أيدي حسابك أسفل اسمك

ℹ️ معلومات:
• أدخل مُعرّف اللاعب، وليس اسم المستخدم
• يتم التحقق من مُعرّف اللاعب تلقائيًا
• اذا اخطأت بأيدي حسابك ستذهب الشدات لحساب اخر و لن يتم استرداد الرصيد
• يستغرق الأمر من 1 إلى 15 دقائق تقريبًا
• مثال على ايدي الحساب مثال: 5123456789

📩 الآن أرسل أيدي حسابك:`;
  orderType = "pubg60";
}

if (type === "PUBG325") {
  cost = pubg325Prices[qty] || 0;
  askText = `⚡️ شحن شدات PUBG Mobile
